...
- 노래방 모드: MR(반주) 버전 재생 + 원곡(보컬 포함)과 유저 음성 비교 채점
- 싱크가사(LRC) 자동 검색 및 실시간 표시 (재생/노래방 모두 지원)
- 한글 명령어 및 설명
- 서버별 볼륨 저장 로직 제거, 실시간 볼륨 조절 및 조회
- 자동 명령어 로더, Graceful Shutdown, 일관된 에러 처리

## Slash 명령어
| 명령 | 설명 |
|------|------|
| `/재생 <제목 또는 URL>` | 음악 재생 (싱크가사 자동 표시) |
| `/일시정지` | 재생/일시정지 토글 |
| `/정지` | 재생 중지 및 음성 연결 해제 |
| `/건너뛰기` | 다음 곡으로 건너뜀 |
| `/대기열` | 재생 대기열 표시 |
| `/현재재생` | 현재 재생 곡 정보 |
| `/볼륨 [0-100]` | 볼륨 조절/조회 (입력 없으면 현재 볼륨 안내) |
| `/노래방 <제목 또는 URL>` | 노래방 MR 재생 + 원곡 비교 채점 + 싱크가사 표시 |
